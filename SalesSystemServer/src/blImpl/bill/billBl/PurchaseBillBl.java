...
/**
 * PurchaseBillBl逻辑类，提供增删改查，草稿创建等相关进货单操作
 * @author 张傲  161250193
 * @version 2017.12.1
 */
...
public class PurchaseBillBl implements PurchaseBillBlService{
	...
	static final BillCategoryEnum BILL_CATEGORY_ENUM = BillCategoryEnum.PURCHASE_BILL;//当前单据的种类
	...
	public boolean createBill(PurchaseBillPO bill) {
		...
		boolean isSuccessedCreation = createBillDataService.createPurchaseBill(bill);
	...
	public boolean submitBill(String billId){
		PurchaseBillPO bill = getBillById(billId);
		if(bill==null){return false;}
		deleteDraftBill(billId);//删除该草稿
		createBill(bill);//创建该进货单的未审批状态单据
		return true;
	}
	...
	public PurchaseBillPO getBillById(String billId) {
		PurchaseBillPO bill = (PurchaseBillPO) getBillBl.getBillByNameAndId(BILL_NAME, billId);
		return bill;
	} 
	...
	public ArrayList<PurchaseBillPO> getBillListByState(BillStateEnum billState) {
		...
		ArrayList<PurchaseBillPO> targetBills = new ArrayList<PurchaseBillPO>();//需要返回的单据清单
		//显式强制类型转换并筛选
		for(BillPO billPO : bills){
			targetBills.add((PurchaseBillPO)billPO);
		}
		return targetBills;
	}
	...
	public ArrayList<PurchaseBillPO> getDraftBillsListByCreater(UserInfoPO targetUserInfo) {
		ArrayList<PurchaseBillPO> bills = getBillListByState(BillStateEnum.DRAFT);
		ArrayList<PurchaseBillPO> targetBills = new ArrayList<PurchaseBillPO>();//需要返回的单据清单
		//显式强制类型转换并筛选
		for(PurchaseBillPO billPO : bills){
	...
	public ArrayList<PurchaseBillPO> getTBDBillsListByCreater(UserInfoPO targetUserInfo) {
		ArrayList<PurchaseBillPO> bills = getBillListByState(BillStateEnum.TBD);
		ArrayList<PurchaseBillPO> targetBills = new ArrayList<PurchaseBillPO>();//需要返回的单据清单
		//显式强制类型转换并筛选
		for(PurchaseBillPO billPO : bills){
			UserInfoPO user = billPO.getCreater();
	...
	public ArrayList<PurchaseBillPO> getToDoBillsListByExecutor(UserInfoPO targetUserInfo) {
		ArrayList<PurchaseBillPO> bills = getBillListByState(BillStateEnum.TODO);
		ArrayList<PurchaseBillPO> targetBills = new ArrayList<PurchaseBillPO>();//需要返回的单据清单
		//显式强制类型转换并筛选
		for(PurchaseBillPO billPO : bills){
	...
	public ArrayList<PurchaseBillPO> getDeniedBillsListByCreater(UserInfoPO targetUserInfo) {
		ArrayList<PurchaseBillPO> bills = getBillListByState(BillStateEnum.DENIED);
		ArrayList<PurchaseBillPO> targetBills = new ArrayList<PurchaseBillPO>();//需要返回的单据清单
		//显式强制类型转换并筛选
		for(PurchaseBillPO billPO : bills){
			UserInfoPO user = billPO.getCreater();
	...
	public ArrayList<PurchaseBillPO> getDoneBillsListByExecutor(UserInfoPO targetUserInfo) {
		ArrayList<PurchaseBillPO> bills = getBillListByState(BillStateEnum.DONE);
		ArrayList<PurchaseBillPO> targetBills = new ArrayList<PurchaseBillPO>();//需要返回的单据清单
		//显式强制类型转换并筛选
		for(PurchaseBillPO billPO : bills){
			UserInfoPO user = billPO.getExecutor();

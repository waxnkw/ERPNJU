...
/**
 * InventoryWarningBillBl逻辑类，提供增删改查，草稿创建等相关库存报警单操作
 * @author 张傲  161250193
 * @version 2017.12.1
 */
...
public class InventoryWarningBillBl implements InventoryWarningBillBlService{
	...
	public boolean submitBill(String billId){
		...
		createBill(bill);//创建该库存报警单的未审批状态单据
		return true;
	}
	...
	public ArrayList<InventoryWarningBillPO> getBillListByState(BillStateEnum billState) {
		...
		ArrayList<InventoryWarningBillPO> targetBills = new ArrayList<InventoryWarningBillPO>();//需要返回的单据清单
		//显式强制类型转换并筛选
	 ...
	 * 专门用于billState部分，比如财务人员读取自己制定的库存报警单草稿

...
/**
 * InventoryOverBillBl逻辑类，提供增删改查，草稿创建等相关库存报溢单操作
 * @author 张傲  161250193
 * @version 2017.12.1
 */
...
public class InventoryOverBillBl implements InventoryOverBillBlService{
	...
	static final BillCategoryEnum BILL_CATEGORY_ENUM = BillCategoryEnum.INVENTORY_OVER_BILL;//当前单据的种类
	...
	public boolean submitBill(String billId){
		...
		createBill(bill);//创建该库存报溢单的未审批状态单据
		return true;
	}
	...
	/**
	 * 根据单据名称和状态读取一系列库存报溢单。<br/>
	 * @param billState 要读取的库存报溢单状态
	 * @return ArrayList<InventoryOverBillPO>读取的库存报溢单列表
	 */
	 @Override
	public ArrayList<InventoryOverBillPO> getBillListByState(BillStateEnum billState) {
		...
		ArrayList<InventoryOverBillPO> targetBills = new ArrayList<InventoryOverBillPO>();//需要返回的单据清单
		//显式强制类型转换并筛选
	...
	/**
	 * 根据单据创建者读取一系列未审批库存报溢单。<br/>
	 * 专门用于billState部分，比如财务人员读取自己制定但是未被审批的单据
	 * @param userName 筛选的单据创建者的名字
	 * @param userId 筛选的单据创建者的Id
	 * @param userRole 筛选的单据创建者的职位
	 * @return ArrayList<InventoryOverBillPO>需要读取的未审批库存报溢单列表
	 */
	 @Override
	...
	/**
	 * 根据单据创建者读取一系列需要执行的库存报溢单。<br/>
	 * 专门用于billState部分,比如财务人员读取需要自己要去执行的单据
	 * @param userName 筛选的单据创建者的名字
	 * @param userId 筛选的单据创建者的Id
	 * @param userRole 筛选的单据创建者的职位
	 * @return ArrayList<InventoryOverBillPO>读取的需要执行的库存报溢单列表
	 */
	 @Override
	...
	/**
	 * 根据单据创建者读取一系列被拒绝的库存报溢单。<br/>
	 * 专门用于billState部分,比如财务人员读取需要自己提交但是审批未通过的单据列表
	 * @param userName 筛选的单据创建者的名字
	 * @param userId 筛选的单据创建者的Id
	 * @param userRole 筛选的单据创建者的职位
	 * @return ArrayList<InventoryOverBillPO>读取的审批未通过的库存报溢单列表
	 */
	 @Override
	...
	/**
	 * 根据单据执行者读取一系列被完成的库存报溢单。<br/>
	 * 专门用于billState部分,比如财务人员读取已经被自己完成的单据列表
	 * @param userName 筛选的单据创建者的名字
	 * @param userId 筛选的单据创建者的Id
	 * @param userRole 筛选的单据创建者的职位
	 * @return ArrayList<InventoryOverBillPO>读取的已经完成的库存报溢单列表
	 */
	 @Override

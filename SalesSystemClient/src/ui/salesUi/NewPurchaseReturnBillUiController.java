...
public class NewPurchaseReturnBillUiController {
	...
	public void init(UserInfoVO user) {
		...
		billIdText.setPromptText("JHTHD-YYYYMMDD-XXXXX");
		...
		try {
			...
			ArrayList<PurchaseReturnBillVO> bills=service.getPurchaseReturnDraftBillsList(user);
			drafts.addAll(bills);
	...
	public void createBillId() {
		...
		if(isValid) {
			...
			billIdText.setText("JHTHD-"+dateTexts[0]+dateTexts[1]+dateTexts[2]+"-"+idText);
		}else {

...
public class ApproveInventoryLossBillUiController {

	
	private static final String DEFAULT_APPROVER_COMENT = "总经理不想理你，并甩给你一张单据";
	private ObservableList<InventoryLossBillVO> bills = FXCollections.observableArrayList();
	private UserInfoVO user;
	private ApproveInventoryLossBillBlService service 
		= RemoteHelperFactory.getApproveBillRemoteHelper().getApproveInventoryLossBillBlService(); 
	 ...
	 * */
	@FXML
	private TableView<InventoryLossBillVO> billTableView;
	@FXML
	private TableColumn<InventoryLossBillVO, String> billIdColumn;
	@FXML
	private TableColumn<InventoryLossBillVO, String> billCreateDateColumn;
	@FXML
	private TableColumn<InventoryLossBillVO, String> billCreaterColumn;
	@FXML
	private TableColumn<InventoryLossBillVO, Button> viewBillColumn;	
	@FXML
	private TableColumn<InventoryLossBillVO, CheckBox> chooseBillColumn;
	@FXML
	private TableColumn<InventoryLossBillVO,Button> remarkTableColumn;
	@FXML
	private TableColumn<InventoryLossBillVO, Button> assignTableColumn;
	...
	private void setViewBillColumn(){
		viewBillColumn.setCellValueFactory(cellDataFeatures->{
			...
			button.setOnMouseClicked(e->{
				InventoryLossBillVO bill = (InventoryLossBillVO)cellDataFeatures.getValue();
	...
	private void setRemarkTableColumn(){
		remarkTableColumn.setCellValueFactory(cellDataFeature->{
			...
			button.setOnMouseClicked(e->{
				InventoryLossBillVO bill = (InventoryLossBillVO)cellDataFeature.getValue();
	...
	private void initBillList(){
		ArrayList<InventoryLossBillVO> inventoryLossBills = new ArrayList<>();
		try {
			inventoryLossBills = service.getBillsList();
		} catch (RemoteException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		bills.addAll(inventoryLossBills);
	}

...
public class ApproveInventoryOverBillUiController {

	
	private static final String DEFAULT_APPROVER_COMENT = "总经理不想理你，并甩给你一张单据";
	private ObservableList<InventoryOverBillVO> bills = FXCollections.observableArrayList();
	private UserInfoVO user;
	private ApproveInventoryOverBillBlService service 
		= RemoteHelperFactory.getApproveBillRemoteHelper().getApproveInventoryOverBillBlService(); 
	 ...
	 * */
	@FXML
	private TableView<InventoryOverBillVO> billTableView;
	@FXML
	private TableColumn<InventoryOverBillVO, String> billIdColumn;
	@FXML
	private TableColumn<InventoryOverBillVO, String> billCreateDateColumn;
	@FXML
	private TableColumn<InventoryOverBillVO, String> billCreaterColumn;
	@FXML
	private TableColumn<InventoryOverBillVO, Button> viewBillColumn;	
	@FXML
	private TableColumn<InventoryOverBillVO, CheckBox> chooseBillColumn;
	@FXML
	private TableColumn<InventoryOverBillVO,Button> remarkTableColumn;
	@FXML
	private TableColumn<InventoryOverBillVO, Button> assignTableColumn;
	...
	private void setViewBillColumn(){
		viewBillColumn.setCellValueFactory(cellDataFeatures->{
			...
			button.setOnMouseClicked(e->{
				InventoryOverBillVO bill = (InventoryOverBillVO)cellDataFeatures.getValue();
	...
	private void setRemarkTableColumn(){
		remarkTableColumn.setCellValueFactory(cellDataFeature->{
			...
			button.setOnMouseClicked(e->{
				InventoryOverBillVO bill = (InventoryOverBillVO)cellDataFeature.getValue();
	...
	private void initBillList(){
		ArrayList<InventoryOverBillVO> inventoryOverBills = new ArrayList<>();
		try {
			inventoryOverBills = service.getBillsList();
		} catch (RemoteException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		bills.addAll(inventoryOverBills);
	}

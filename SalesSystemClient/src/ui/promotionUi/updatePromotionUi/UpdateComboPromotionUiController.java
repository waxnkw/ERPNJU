...
public class UpdateComboPromotionUiController implements PromotionUiForGiftsUiController{
	private PromotionUiController promotionUiController;//持有他的controller的引用
	private ComboStrategyVO comboStrategyVO = new ComboStrategyVO();//此界面要显示的ComboStrategy信息
	...
	protected void ensureButtonListener(Event event){
		...
		promotionUiController.updateComboStrategy(comboStrategyVO);
	...
	protected void init(){
		...
		setTodayPromptText();
		setGiftsTableView();
		if(comboStrategyVO!=null){
			setComboText();
		}
	}
	
	protected void setComboStrategyVO(ComboStrategyVO vo){
		this.comboStrategyVO = vo;
	}
	
	protected void setComboText(){
		nameTextField.setText(comboStrategyVO.getName());
		...
		discountTextField.setText(comboStrategyVO.getDiscount()*100+"");
